...
class Management extends CI_Controller {
	...
	public function users()
	{
		...
		$crud->columns('username','email','real_name','active', 'id');
		$crud->fields('username','email','password','verify_password', 'real_name','active', 'id');

		$crud->set_rules('email', 'Email', 'required|email|is_unique[users.email]');

		//$crud->field_type('aktif','dropdown', array('0'=>'Tidak Aktif', '1'=>'Aktif'));		
		...
		if($state != 'edit' && $state != 'update' && $state != 'update_validation'){
			...
			$crud->set_rules('password', 'Password', 'matches[verify_password]'); 
		}
		else{
			$crud->set_rules('verify_password', 'Verify Password');
			$crud->set_rules('password', 'Password', 'matches[verify_password]'); 
